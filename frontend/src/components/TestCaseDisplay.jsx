...
function TestCaseDisplay({ testCases, onDownloadCSV, onDownloadJSON }) {
  ...
  const [view, setView] = useState('index') // 'index' or 'details'
  ...
  const expandAll = () => {
    setExpandedIds(testCases.map((testCase) => testCase.id))
  }
    ...
    <div className="test-case-display">
      <div className="display-header">
        ...
        <div className="header-actions">
          <div className="result-downloads">
            <button className="result-action-btn" onClick={onDownloadCSV}>
              Download CSV
            </button>
            <button className="result-action-btn" onClick={onDownloadJSON}>
              Download JSON
            </button>
          </div>
          <div className="view-toggle">
            ...
            <button
              className={`view-btn ${view === 'details' ? 'active' : ''}`}
      ...
      {testCases.length === 0 ? (
        <div className="empty-state">
          <p>No test cases generated yet. Upload a document to generate.</p>
        </div>
      ) : view === 'index' ? (
        /* INDEX TABLE VIEW */
            ...
            <tbody>
              {testCases.map((tc, idx) => (
                <tr key={idx} onClick={() => { setView('details'); setExpandedIds([tc.id]); }}>
                  ...
                  <td>
                    <span className="priority-badge" style={{ backgroundColor: getPriorityColor(tc.priority) }}>
                      {tc.priority}
                    </span>
                  </td>
                </tr>
